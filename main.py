...
def main():
    # Load the YOLO model
    model = YOLO("yolov8x.pt")  # Use a larger model for better accuracy
    ...
    clock_overlay = None  # Clock overlay

    while True:
        success, img = cap.read()
        
        if clock_overlay is None:
            clock_overlay = draw_clock_overlay(img, center_x, center_y, radius)
        
        img = cv2.addWeighted(img, 1, clock_overlay, 0.3, 0)  # Add clock overlay with some transparency
